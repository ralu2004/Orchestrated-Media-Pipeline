...
- **`ProgressReporter`** — implemented by **`ConsoleProgressReporter`** (CLI timestamps and durations) and **`NoOpProgressReporter`** (null-object style: no output, useful for tests or headless runs).
...
- **BPP / encoding profile** — bits-per-pixel from container metadata approximates visual complexity cheaply before nine transcodes, avoiding a full decode for per-frame entropy.
- **Shared analysis probes** — ffmpeg filters run once into **`RawAnalysisData`** so parsers share one subprocess snapshot.
- **Sequential vs parallel** — phases stay ordered where outputs are real prerequisites (ingest → analysis → compliance → packaging); only **visuals** and **audio** overlap under **Processing**.
...
- **Per-phase contexts** — narrow records avoid a god object and make dependencies explicit.
